...
public class GoodAI extends Player {
   ...
   public static class AIConstants {
      ...
      public AIConstants() {
         ...
         */
         ...
         MIN_DEFENSIVE_DEFENSE = 13;
         AGGRESSION = 0.1965957281144417;
         BASE_DISTANCE_FACTOR = 688.9222691138962;
         DISTANCE_WEIGHTING = 0.5073434974680578;
         ...
         */
         ...
         MIN_DEFENSIVE_DEFENSE = 17;
         AGGRESSION = 2.0844474549539864;
         BASE_DISTANCE_FACTOR = 522.294342150184;
         DISTANCE_WEIGHTING = 0.44668924816334477;
      ...
      public AIConstants(boolean umf, boolean ovc, int mad, int mdd, double ag, double bdf, double dw, double ucpr, double ugpr, double csm) {
      ...
      public AIConstants(AIConstants a, AIConstants b) {
         ...
         USE_OLD_VALUE_CALCULATION  = getCombination(rnd.nextInt(2), a.USE_OLD_VALUE_CALCULATION, b.USE_OLD_VALUE_CALCULATION);
      ...
      public static AIConstants generateRandomConstants() {
         Random rnd = new Random();
         return new AIConstants(rnd.nextBoolean(), rnd.nextBoolean(), rnd.nextInt(100), rnd.nextInt(100), rnd.nextDouble() * 3, 
               rnd.nextDouble() * 1000, rnd.nextDouble() * 3, rnd.nextDouble(), 
   ...
   protected void turn() {
      ...
      if (!danger.isEmpty()) {
         ...
         if (danger.stream().anyMatch(planet -> planet.ownedBy(this))) {
            p = danger.stream().filter(planet -> planet.ownedBy(this)).min((a,b) -> Integer.compare(a.getNumUnits(), b.getNumUnits())).get();
         } else if (danger.stream().anyMatch(planet -> planet.ownedByOpponentOf(this))) {
            p = danger.stream().filter(planet -> planet.ownedByOpponentOf(this)).min((a,b) -> Integer.compare(a.getNumUnits(), b.getNumUnits())).get();
         } else {
      ...
      if (defending) {
         ...
         if (target != null) {
            ...
            for (Planet p : myPlanets.stream().sorted((Planet a, Planet b) -> Double.compare(new Location(a).distance(finalTarget), new Location(b).distance(finalTarget))).collect(Collectors.toList())) {
               if (p != target) {
                  int contribution = p.getNumUnits() - PlayerUtils.getIncomingFleetCount(p, fleets) - constants.MIN_DEFENSIVE_DEFENSE;
   ...
   public void moveFleetsForwards() {
      ...
      if (theirPlanets.size() > 0) {
         ...
         Optional<Planet> target = mine.stream().min((a, b) -> Double.compare(theirLocation.distance(a), theirLocation.distance(b)));
         if (target.isPresent()) {
   ...
   private void contest() {
      ...
      if (take != null) {
         int toSendToTake = 0;
         while (!isEventualOwner(take, (int) Math.ceil(myPlanets.get(0).distanceTo(take) / Fleet.FLEET_SPEED), toSendToTake)) {
            toSendToTake++;
         }
         if (toSendToTake > 0) {
      ...
      for (Fleet fleet : PlayerUtils.getOpponentsFleets(fleets, this)) {
         if (retake.contains(fleet.DESTINATION)) {
            ...
            if (distance > fleetDistance) {
               int toSend = 0;
               while (!isEventualOwner(fleet.DESTINATION, distance, toSend)) toSend++;
               if (toSend > 0) {
   ...
   private double getInfluence(Planet a, Planet b, Location opponent) {
      if (a == b) {
         ...
         if (constants.USE_OLD_VALUE_CALCULATION) {
            return a.getNumUnits() * 0.5 * (new Location(a).distance(b) + constants.BASE_DISTANCE_FACTOR) / (opponent.distance(b) + constants.BASE_DISTANCE_FACTOR);
         } else {
      ...
      for (Planet influencing : myPlanets) {
         for (Planet p : planets) {
            if (influencing == p) {
               ...
               double distanceFactor = theirDistance / (myDistance + theirDistance);
               double influence = influencing.getNumUnits() * Math.pow(distanceFactor, constants.DISTANCE_WEIGHTING);
         ...
         if (gameCount == GAMES_PER_SAMPLE) {
            ...
            if (position >= currentGeneration.size()) {
               ...
               for (int i = 0; i < POOL_SIZE / 2; i++) {
                  ...
                  nextGeneration.add(new AIConstants(currentGeneration.get(i), AIConstants.generateRandomConstants()));
               }
