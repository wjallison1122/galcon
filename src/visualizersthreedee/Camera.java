...
public class Camera {
    ...
    private PlanetHolder[] drawList;
    private Vector lateral = new Vector();
    private Vector horizontal = new Vector();
    private ArrayList<PlanetHolder> suns;
    private Vector mouse = new Vector(-100,-100,0);
    private PlanetHolder nearestToMouse;
    private PlanetObject following;

    public Camera(Vector _location) {
        location = _location;
    }

    public void draw(Planet[] objectList, Graphics g, int x, int y) {
        if (following != null)
            location = Vector.difference(new Vector(), following.location);
        precalcDrawing();
        drawList = new PlanetHolder[objectList.length];
        suns = new ArrayList<>();
        for (int i = 0; i < objectList.length; i++) {
            PlanetHolder h = new PlanetHolder(objectList[i]);
    ...
    private void calcBasicCoords(PlanetHolder gh) {
    ...
    private void scaleForDistance(PlanetHolder gh) {
    ...
    private static void quickSort(PlanetHolder arr[], int left, int right) {
    ...
    public void drawSunArc(PlanetHolder gh, PlanetHolder s, Graphics g, int x, int y) {
        //bugged
    ...
    public PlanetHolder getNearestToMouse()
    {
        return nearestToMouse;
    }
    
    public void setFollowing(PlanetObject go){
        following = go;
    }
    
    public PlanetObject getFollowing(){
        return following;
    }
}
